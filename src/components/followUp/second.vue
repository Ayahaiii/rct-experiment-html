    ...
    <div class="ware">
      <div class="pcit">
        ...
        <el-table :data="tableData" border style="width: 100%">
          <el-table-column prop="sampleId" label="样本编号" width="153"></el-table-column>
          <el-table-column prop="age" label="年龄" width="153"></el-table-column>
          <el-table-column  label="性别" width="153">
              <template slot-scope="scope">
                                      <span v-if="scope.row.sex==1">男</span>
                                      <span v-if="scope.row.sex==2">女</span>
               </template>
          </el-table-column>
          <el-table-column prop="measurementScore" label="初始测量分数" width="153">
              <template slot-scope="scope">
          ...
          <el-table-column prop="firstAccessScore" label="第6月随访数据" width="153">
              <template slot-scope="scope">
                                      <span v-if="scope.row.firstAccessScore==0">失访</span>
                                      <span v-if="scope.row.firstAccessScore==null">女</span>
                                      <span v-if="scope.row.firstAccessScore>0">{{scope.row.firstAccessScore}}</span>
               </template>
          </el-table-column>
          <el-table-column prop="secondAccessScore" label="第18月随访数据" width="153">
              <template slot-scope="scope">
                                      <span v-if="scope.row.secondAccessScore==0">失访</span>
                                      <span v-if="scope.row.secondAccessScore==null"></span>
                                      <span v-if="scope.row.secondAccessScore>0">{{scope.row.secondAccessScore}}</span>
               </template>
          </el-table-column>
        </el-table>
      </div>
      <div class="tau">
        ...
        <el-table :data="tableData2" align="center" border style="width: 100%">
          <el-table-column prop="sampleId" label="样本编号" width="153"></el-table-column>
          <el-table-column prop="age" label="年龄" width="153"></el-table-column>
          <el-table-column prop="sex" label="性别" width="153">
               <template slot-scope="scope">
                      <span v-if="scope.row.sex==1">男</span>
                      <span v-if="scope.row.sex==2">女</span>
               </template>
      ...
      <div class="footer"> 
        <el-button type="primary" class="bottom-right" style="outline:none;cursor:pointer" @click="sendvisit" v-if="!disable" >开始第二次随访</el-button>
        <el-button type="primary" class="bottom-left" style="outline:none;cursor:pointer" @click="next" v-if="nextNum==2">下一步</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
    ...
    this.httpPost("/rct/v1/searchGroupResult", param).then(res => {
      ...
      let diableNum=0;
       this.tableData.forEach((item) => {
         if(item.secondAccessScore && item.secondAccessScore>0){
  ...
  methods: {
    ...
    checkVideoFun(videos) {
      ...
      this.httpPost("/rct/v1/getSecondAccessFlash", param).then(res => {
    ...
    sendvisit() {
      ...
      this.httpPost(
        "/rct/v1/updateUserTaskSampleListSecondAccessScore",
        param
      ).then(res => {
         this.$message({
          message: '第二次随访成功',
